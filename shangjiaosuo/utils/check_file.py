...
from shangjiaosuo.settings import APPENDIX_DIR, LOG_SIZE, APP_DIR_SIZE
import os
import re
...
def check_log(log_name):
    ...
    global sse_log_index, sse_cp_log_index
    this_path = os.getcwd()
    ...
    if log_file_size >= LOG_SIZE:
        if log_name == 'sse_notice.log':
            new_log_name = log_name.split('.')[0] + str(sse_log_index) + '.' + log_name.split('.')[1]
            sse_log_index += 1
        if log_name == 'sse_cp_notice.log':
            new_log_name = log_name.split('.')[0] + str(sse_cp_log_index) + '.' + log_name.split('.')[1]
            sse_cp_log_index += 1
